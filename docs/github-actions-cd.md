...
`dev` environment의 **Deployment branches and tags**를 **Selected branches and tags**로 설정하고 `release` 브랜치만 허용한다. 이 설정은 필수다. Environment를 사용하는 OIDC `sub` claim에는 브랜치가 포함되지 않으므로, 이 보호 규칙이 없으면 다른 브랜치의 워크플로도 `environment: dev`를 지정해 배포 Role을 맡을 수 있다.
